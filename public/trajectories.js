...
function getAnglesByPoint(x, y) {
    var rel =
        radius -
        Math.sqrt(
            (x - antennaX) * (x - antennaY) + (y - antennaY) * (y - antennaY)
        );
    ...
    if (x <= antennaX)
        az =
            360 -
            Math.abs(Math.atan2(x - antennaX, antennaY - y) * (180 / Math.PI));
...
function drawTrajectory(ctx, name, traj) {
    ...
    ctx.quadraticCurveTo(traj[i].x, traj[i].y, traj[i + 1].x, traj[i + 1].y);
...
function drawTrajectories(ctx) {
    ...
    if (needToDrawNext) {
        var minutesToNext = Math.floor((minNextTime - Date.now()) / 1000 / 60);
        var secondsToNext = Math.floor((minNextTime - Date.now()) / 1000) % 60;
